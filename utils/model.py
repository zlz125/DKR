...
def prepare_sequence_posttrain(args):
    ...
    if 'dga' in args.baseline:
        ...
        # args.softmask_compute = 'before_distill_after_mlm_before_mlm'
        args.softmask_compute = 'before_distill_after_mlm_curr_mlm'
        #TODO
        #args.layer_to_mask = 'head_mask_intermediate_mask_output_mask'
        args.layer_to_mask = 'q_mask_k_mask_v_mask_intermediate_mask_output_mask'
    ...
    # output = f'{args.base_dir}/seq{args.idrandom}/{args.max_samples}samples/{args.baseline}'
    imp_key_output = f'{args.base_dir}/seq{args.idrandom}/{args.max_samples}samples/{args.baseline}/impt/key/'
    imp_query_output = f'{args.base_dir}/seq{args.idrandom}/{args.max_samples}samples/{args.baseline}/impt/query/'

    # if args.pt_task==5:
    #     output_after = f'{args.base_dir}/seq{args.idrandom}/{args.max_samples}samples/{args.baseline}/last_roberta/'
    # else:
    #     output_after=f'{args.base_dir}/seq{args.idrandom}/{args.max_samples}samples/{args.baseline}/{data[args.pt_task+1]}_roberta/'
    ...
    if 'dga' in args.baseline or 'das' in args.baseline:
        ...
        if args.softmask_compute is not None:
            ...
            #     for pre_t in range(args.pt_task + 1):
            #         new_dir = f'{output}before_mlm{pre_t}/'
            #         print("new_dir:"+new_dir)
            #         os.makedirs(new_dir, exist_ok=True)
            #     saved_output = f'{output_dir}/{data[0]}_roberta/before_mlm0/'
            #     args.saved_output_dir += [saved_output] # only used in the first one (for the importance of general knoweledge)

            if 'before_distill' in args.softmask_compute and 'one' not in args.baseline and args.pt_task==0:
                new_dir=f'{output}before_distill{args.pt_task}/'
                os.makedirs(new_dir,exist_ok=True)
                args.saved_output_dir += [f'{output_dir}/{data[args.pt_task]}_roberta/before_distill{args.pt_task }/']
            ...
            # if 'after_mlm' in args.softmask_compute and args.pt_task==0:
            #     new_dir = f'{output_after}after_mlm{args.pt_task}/'
            #     os.makedirs(new_dir, exist_ok=True)
                # args.saved_output_dir += [f'{output_dir}/{data[t+1]}_roberta/after_mlm{t}/' for t in range(args.pt_task)]
                # args.saved_output_dir += [f'{output_dir}/{data[args.pt_task + 1]}_roberta/after_mlm{args.pt_task}/']

            if 'after_mlm' in args.softmask_compute:
                new_dir = f'{output}after_mlm{args.pt_task}/'
                os.makedirs(new_dir, exist_ok=True)
                args.saved_output_dir += [f'{output_dir}/{data[t]}_roberta/after_mlm{t}/' for t in range(args.pt_task)]
                # args.saved_output_dir += [f'{output_dir}/{data[args.pt_task]}_roberta/after_mlm{args.pt_task-1}/']

            if 'before_mlm' in args.softmask_compute:
                new_dir = f'{output}before_mlm{args.pt_task}/'
                os.makedirs(new_dir, exist_ok=True)
                # args.saved_output_dir += [f'{output_dir}/{data[t]}_roberta/before_mlm{t}/' for t in range(args.pt_task)]
                args.saved_output_dir += [f'{output_dir}/{data[args.pt_task]}_roberta/before_mlm{args.pt_task}/']

            if 'curr_mlm' in args.softmask_compute:
                new_dir = f'{output}curr_mlm{args.pt_task}/'
                os.makedirs(new_dir, exist_ok=True)
                # args.saved_output_dir += [f'{output_dir}/{data[t+1]}_roberta/after_mlm{t}/' for t in range(args.pt_task)]
                # args.saved_curr_output_dir = new_dir
    ...
    args.saved_curr_output_dir=f'{output}curr_mlm{args.pt_task}/'
    args.output_dir = output
    args.impt_key_output_dir = imp_key_output
    args.impt_query_output_dir = imp_query_output
    # args.output_dir_after=output_after
    ...
    if args.pt_task == 0 or 'one' in args.baseline or ('wiki' in args.baseline and args.pt_task==1): # no pre-trained for the first
        args.model_name_or_path = "/home/zlz/codes/LLMs/ContinualLM-main_kv_grad/roberta-base"
    else:
...
#根据训练类型初始化一个预训练的Roberta模型，创建一个不更新参数的 teacher 模型，并将这两个模型传递给一个自定义的 MyModel 类。
#这可能是用于实现某种形式的知识蒸馏或模型集成的设置。
def _lookfor_model_others(args,training_type):
    ...
    if args.finetune_class_num is not None:
        num_labels=args.finetune_class_num
    else:
        num_labels=args.class_num
